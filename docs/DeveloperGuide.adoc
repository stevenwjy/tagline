...
image::ContactShowExecutionDiagram.png[]
...
image::RetrieveAssociatedTagsDiagram.png[]
...
.. Prerequisites: List all persons using the `contact list` command. Multiple persons in the list.
.. Test case: `contact delete 1` +
   Expected: Contact with id 1 is deleted from the list. Details of the deleted contact shown in the status message.
.. Test case: `contact delete 0` +
   Expected: No contact is deleted. Error details shown in the status message.

=== Showing a contact profile

. Showing a contact profile that contains contact information and notes that are associated with the contact.
.. Test case: `contact show 1` +
   Expected: Showed the profile for contact with id 1.
.. Test case: `contact show 0` +
   Expected: No contact is showed. Error details shown in the status message.
